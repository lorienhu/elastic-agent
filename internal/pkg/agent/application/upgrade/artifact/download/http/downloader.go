...
func (e *Downloader) Download(ctx context.Context, a artifact.Artifact, uri string) (_ string, err error) {
	...
	path, err := e.downloadFile(ctx, uri, a.FilePath)
	downloadedFiles = append(downloadedFiles, path)
	if err != nil {
		return "", err
	}

	hashPath, err := e.downloadFile(ctx, uri+".sha512", a.FilePath+".sha512")
