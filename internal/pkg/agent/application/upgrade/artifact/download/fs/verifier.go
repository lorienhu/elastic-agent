...
// location against a key stored on elastic.co website.
func (v *Verifier) Verify(ctx context.Context, a artifact.Artifact, _ string, skipDefaultPgp bool, pgpBytes ...string) error {
	if err := download.VerifySHA512HashWithCleanup(v.log, a.FilePath); err != nil {
		return fmt.Errorf("failed to verify SHA512 hash: %w", err)
	}

	if err := v.verifyAsc(a.Filename, a.FilePath, skipDefaultPgp, pgpBytes...); err != nil {
		var invalidSignatureErr *download.InvalidSignatureError
		if errors.As(err, &invalidSignatureErr) {
			if err := os.Remove(a.FilePath + ".asc"); err != nil {
